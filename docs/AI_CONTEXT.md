...
- **Theming**: Uses CSS custom properties on `:root` / `[data-theme="dark"]` in `src/index.css`, driven by `detectColorScheme` (`shared/utilities/detect-color-scheme.utility.ts`, called pre-render from `app-shell.ts`) and `brew-theme-toggle` (`shared/stores/theme.store.ts`).
