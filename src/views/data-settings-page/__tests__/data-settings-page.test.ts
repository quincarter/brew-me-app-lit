...
import "../data-settings-page";
import type { DataSettingsPage } from "../data-settings-page";
...
describe("data-settings-page", () => {
  ...
  beforeEach(async () => {
    ...
    element = document.createElement("data-settings-page") as DataSettingsPage;
  ...
  it("renders an Export data button", () => {
  ...
  it("renders an Import data button", () => {
  ...
  describe("Danger zone", () => {
    const deleteAllDataButton = (): Element | undefined =>
      Array.from(element.shadowRoot?.querySelectorAll("brew-button") ?? []).find(
        (button) => button.textContent?.trim() === "Delete all data",
      );

    const confirmDeleteButton = (): Element | undefined =>
      Array.from(element.shadowRoot?.querySelectorAll("brew-button") ?? []).find(
        (button) => button.textContent?.trim() === "Yes, delete everything",
      );

    it("shows a confirm step instead of deleting immediately", async () => {
      clickButton(deleteAllDataButton());
      await element.updateComplete;

      expect(confirmDeleteButton()).not.toBeUndefined();
    });

    it("deletes all saved brews and custom brew types when confirmed", async () => {
      ...
      expect(confirmDeleteButton()).toBeUndefined();
      expect(savedBrewsSignal.value).toEqual([]);
      expect(customBrewTypesSignal.value).toEqual([]);
    });
  });
});
