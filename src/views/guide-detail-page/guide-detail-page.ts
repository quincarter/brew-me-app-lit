...
export class GuideDetailPage extends SignalWatcher(LitElement) {
  ...
  protected willUpdate(): void {
    const guide =
      BREW_GUIDE.find((item) => item.id === this.routeParams.id) ??
      BREW_GUIDE[0];
  ...
  render(): HTMLTemplateResult {
    const guide =
      BREW_GUIDE.find((item) => item.id === this.routeParams.id) ??
      BREW_GUIDE[0];
    const savedMatches = savedBrewsSignal.value.filter(
      (brew) => brew.brewType === guide.name,
    );
      ...
      <div class="screen">
        ...
        <div class="content">
          ...
          <div class="ai-tip">
            ...
            <p class="ai-tip-body">
              ${guide.aiTips[this._aiTipIndex % guide.aiTips.length]}
            </p>
          </div>
            ...
            : html`
                <div class="no-match">
                  ...
                  <brew-button
                    variant="filled"
                    @button-click="${() =>
                      this._calcThisRatio(guide.ratioDefault)}"
          ...
          <brew-video-search
            title="${guide.videos.length > 0
              ? "Looking for more?"
              : "Watch a walkthrough"}"
            subtitle="Find ${guide.videos.length > 0
              ? "more "
