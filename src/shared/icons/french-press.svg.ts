import { svg } from "lit";

export const FRENCH_PRESS_ICON = svg`
<svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
          <rect x="7" y="7" width="10" height="12" rx="1"></rect>
          <path d="M8 7h8"></path>
          <circle cx="12" cy="4.5" r="1"></circle>
          <path d="M12 5.5v1.5"></path>
          <path d="M12 8v11"></path>
          <path d="M17 10c2 0 2.5 1.5 2.5 3s-.5 3-2.5 3"></path>
          <path d="M7 8.5c-1 0-1.6.4-2 1.2"></path>
          <path d="M9 19v1M15 19v1"></path>
        </svg>`;
