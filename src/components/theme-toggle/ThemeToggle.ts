 ...
 * `detectColorScheme` (`detect-color-scheme.utility.ts`) can apply it before
 * paint on the next load - the Settings screen's dark-mode switch
 * reads/writes the same store, so the two always agree.
